...
"""
*/
async function decodeSequence(inputSeq, encoderModel, decoderModel, numDecoderTokens, targetBeginIndex, reverseTargetCharIndex, maxDecoderSeqLength) {
    ...
    while (!stopCondition) {
        ...
        // Sample a token
        const sampledTokenIndex = await outputTokens.squeeze().argMax(-1).array();
...
async function main() {
    ...
    const { inputTexts, maxDecoderSeqLength, numEncoderTokens, numDecoderTokens, targetTokenIndex, encoderInputData, decoderInputData, decoderTargetData, } = await readData(args.data_path);
    ...
    for (let seqIndex = 0; seqIndex < args.num_test_sentences; seqIndex++) {
        ...
        const decodedSentence = await decodeSequence(inputSeq, encoderModel, decoderModel, numDecoderTokens, targetBeginIndex, reverseTargetCharIndex, maxDecoderSeqLength);
        console.log('-');
