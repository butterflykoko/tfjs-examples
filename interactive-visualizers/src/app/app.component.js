...
let AppComponent = (() => {
    ...
    var AppComponent = class {
        ...
        modelDisplayName = DEFAULT_MODEL_DISPLAY_NAME;
        modelFormat = null; // Either 'tflite' or 'tfjs'.
        tfWebApiName = null;
        tfWebApi = null;
        modelMetadataUrl = null;
        modelMetadata = null;
        model = null;
        labelmap = null;
        defaultScoreThreshold = 0.0;
        publisherName = null;
        publisherThumbnailUrl = null;
        // Query related variables.
        queryImageHeight = null;
        queryImageWidth = null;
        // Test data related variables.
        testImagesIndexUrl = null;
        testImages = [];
        uploadedImages = [];
        ...
        hoveredDetectionLabel = null;
        hoveredDetectionResultLabel = null;
        hoveredDetectionResultId = null;
        // Segmenter specific variables.
        segmenterPredictions = null;
        segmenterLabelList = null;
        hoveredSegmentationLabel = null;
        ngOnInit() {
         ...
         */
        async initAppWithTfliteModel(tfliteModelUrl) {
            ...
            await this.tfWebApi.init(environment_1.environment.tfWebWasmFilesPrefix + this.tfWebApiName + '/', tfliteModelUrl);
         ...
         */
        async initAppWithTfjsModel(modelMetadataUrl) {
            // Load model & metadata.
         ...
         */
        async testImageSelected(imageUrl, index) {
            try {
         ...
         */
        async handleInputImage(imageDataURL, index) {
         ...
         */
        async fetchLabelmap(labelmapName) {
            try {
         ...
         */
        async runImageClassifier(image, index) {
            let results = [];
            if (this.modelFormat === 'tflite') {
                ...
                // Execute the model.
                const startTs = Date.now();
                const outputTensor = await this.model.executeAsync(imageTensor);
                this.resultsLatency = Date.now() - startTs;
                tf.dispose(imageTensor);
                const squeezedOutputTensor = outputTensor.squeeze();
                tf.dispose(outputTensor);
                const predictions = await squeezedOutputTensor.array();
         ...
         */
        async runImageSegmenter(image, index) {
            let predictions = [];
            if (this.modelFormat === 'tflite') {
                const startTs = Date.now();
                ...
                const startTs = Date.now();
                const outputTensor = await this.model.executeAsync(imageTensor, outputTensorName);
                this.resultsLatency = Date.now() - startTs;
                tf.dispose(imageTensor);
                const squeezedOutputTensor = outputTensor.squeeze();
                tf.dispose(outputTensor);
                predictions = await squeezedOutputTensor.array();
                tf.dispose(squeezedOutputTensor);
                // Fetch the labelmap.
                if (this.labelmap == null && outputHeadMetadata.labelmap_path != null) {
                    await this.fetchLabelmap(outputHeadMetadata.labelmap_path);
         ...
         */
        async runObjectDetector(image, index) {
            const results = [];
            if (this.modelFormat === 'tflite') {
                const startTs = Date.now();
                ...
                const numDetections = await squeezedNumDetections.array();
                const detectionBoxes = await squeezedDetectionBoxes.array();
                const detectionScores = await squeezedDetectionScores.array();
                const detectionClasses = await squeezedDetectionClasses.array();
